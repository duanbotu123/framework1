...
inline int checkMeshMode(Mesh& mesh)
{
    int mesh_mode;
    ...
    {
        for (auto fe_it = mesh.cfe_cwbegin(*fIt); fe_it.is_valid(); ++fe_it)
        {
            ++count;
        }
        if(count == 4)
...
inline void printBasicMeshInfo(Mesh& mesh)
{
    ...
    printf("Vertex : %ld; Face : %ld; Edge : %ld\n\n",
        mesh.n_vertices(),mesh.n_faces(),mesh.n_edges());
}

inline bool read_data(const std::string filename, Mesh& mesh)
{
...
inline bool write_data(const char* filename, Mesh& mesh, Scalar scale)
{
    for (Mesh::VertexIter v_it = mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it)
    {
...
inline std::string num2str(int num, const int size, bool is_add0)
{
    char s_id[10]={0};
    s_id[size] = '\0';
    int pos = 0;
